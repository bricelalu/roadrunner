...
import (
	...
	"github.com/spiral/roadrunner/v2/plugins/broadcast/memory"
	"github.com/spiral/roadrunner/v2/plugins/broadcast/redis"
...
// Init service.
func (s *Service) Init(cfg *redis.Config, rpc *rpcPlugin.Plugin) (ok bool, err error) {
	s.cfg = cfg

	if s.cfg.Redis != nil {
		if s.broker, err = redis.redisBroker(s.cfg.Redis); err != nil {
			return false, err
		}
	} else {
		s.broker = memory.memoryBroker()
	}
