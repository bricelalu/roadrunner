...
func TestRPC_Broadcast(t *testing.T) {
	...
	assert.NoError(t, rcpClient.Call(
		"broadcast.Publish",
		[]*broadcast.Message{newMessage("topic", `"hello1"`)},
		&ok,
	))
	assert.True(t, ok)
	...
	assert.NoError(t, rcpClient.Call(
		"broadcast.Publish",
		[]*broadcast.Message{newMessage("topic", `"hello2"`)},
		&ok,
	))
	assert.True(t, ok)
	...
	assert.NoError(t, rcpClient.Call(
		"broadcast.Publish",
		[]*broadcast.Message{newMessage("topic", `"hello3"`)},
		&ok,
	))
	assert.True(t, ok)
	...
	assert.NoError(t, rcpClient.Call(
		"broadcast.Publish",
		[]*broadcast.Message{newMessage("topic", `"hello4"`)},
		&ok,
	))
	assert.True(t, ok)
	...
	assert.NoError(t, rcpClient.Call(
		...
		[]*broadcast.Message{newMessage("topic", `"hello5"`)},
		&ok,
	))
	assert.True(t, ok)
